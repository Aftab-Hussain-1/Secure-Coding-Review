...
def login():
    ...
    if not check_password_hash(row["password_hash"], password_value):
        return "Login failed", 401
...
def transfer_funds():
    ...
    return "Transferred {} to {}".format(escape(amount), escape(to_account))
...
def redirect_user():
    redirect_target = request.args.get("url", "/")
    if not _is_safe_redirect_target(redirect_target):
        redirect_target = "/"
